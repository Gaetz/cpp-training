...
void StateManager::draw()
{
    ...
    if (mStates.back().second->isTransparent() && mStates.size() > 1)
    {
        ...
        for (; itr != mStates.end(); ++itr)
        {
            mContext->mWindow->getRenderWindow()->setView(itr->second->getView());
            itr->second->draw();
        }
    }
    // Only one state
...
void StateManager::switchTo(const StateType &type)
{
    ...
    for (auto itr = mStates.begin(); itr != mStates.end(); ++itr)
    {
        ...
        if (itr->first == type)
        {
            ...
            // New state activation and view setup
            tmpState->activate();
            mContext->mWindow->getRenderWindow()->setView(tmpState->getView());
            return;
        }
    }
